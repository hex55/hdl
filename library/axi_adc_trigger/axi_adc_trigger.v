...
module axi_adc_trigger #(
  ...
  reg signed [15-SB:0]  hyst_a_high_limit;
  reg signed [15-SB:0]  hyst_a_low_limit;
  ...
  reg signed [15-SB:0]  hyst_b_high_limit;
  reg signed [15-SB:0]  hyst_b_low_limit;
  ...
  assign data_a_cmp   = data_a[15-SB:0];
  assign data_b_cmp   = data_b[15-SB:0];
  assign limit_a_cmp  = limit_a[15-SB:0];
  assign limit_b_cmp  = limit_b[15-SB:0];
  ...
  always @(posedge clk) begin
    if (data_valid_a == 1'b1) begin
      hyst_a_high_limit <= limit_a_cmp + hysteresis_a[15-SB:0];
      hyst_a_low_limit  <= limit_a_cmp - hysteresis_a[15-SB:0];
  ...
  always @(posedge clk) begin
    if (data_valid_b == 1'b1) begin
      hyst_b_high_limit <= limit_b_cmp + hysteresis_b[15-SB:0];
      hyst_b_low_limit  <= limit_b_cmp - hysteresis_b[15-SB:0];
